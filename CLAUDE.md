...
the dots — so the user sees what matters and decides for themselves. We organize and
explain; we never recommend. Every derived number is framed as a soft sense of direction
with how/what/caveat shown, never as fact or instruction. (Condensed copy lives in the
...
Never start over. Always build on what already exists. This system has
20+ sessions of work behind it — extend, don't rewrite.
SPUS is a normal ETF. It is a Sharia-compliant ETF, but it gets NO special
...
No shorts, no options. Long positions and ETFs only. (Platform constraint —
do not hardcode a specific broker name.)
UAE investor — zero capital gains tax. Factor this into position sizing;
don't let tax thinking block a good decision.
All Telegram output must be Arabic. Dashboard is EN/AR toggle (default EN).
Explain before doing. Describe the change, then make it. Owner is
...
Multi-user — ?nickname= support on portfolio-for-ai.js (mind the 12-fn limit)
Railway migration — replace Make.com orchestration (multi-session effort)
Automated dashboard testing
Short-interest data source
Dashboard footer disclaimer text
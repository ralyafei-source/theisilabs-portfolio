...
const TERMS = {
  archetype: {
    momentum_trap: { label:'فخ الزخم', hint:'قوي سعرياً، ضعيف جوهرياً',
      full:'السهم صاعد بقوة بسبب الزخم، بس اداء الشركة ضعيف — يعني حركة السعر مو بالضرورة شركة قويه. تعتبر أقرب لفرصة تداول من ان تكون استثمار طويل الاجل.' },
    ...
    hidden_quality_out_of_favor: { label:'جودة مهملة', hint:'أساسيات قوية وزخم هادئ',
      full:'أساسيات قوية وزخم هادئ — شركة ادائها قوي بس السوق ما يطاردها حالياً. الشركة جيده ولكن الاهتمام ضعيف، مو سهم ضعيف.' },
    hidden_quality: { label:'جودة قوية', hint:'أساسيات قوية بلا نقطة ضعف',
      full:'أساسيات قوية بلا نقطة ضعف — شركة ادائها قوي والزخم لا يزال جيداً. الجودة حقيقية.' },
...
function buildDeepReadPrompt(sym, o, allRecs, extras){
...
// ── Optional catalyst + news enrichment (Session 35) — only if provided ──
  // extras = { catalysts:[{type,detail,date,age_days}], news:[title,...] }
  const cat = (extras && Array.isArray(extras.catalysts)) ? extras.catalysts : [];
  const news = (extras && Array.isArray(extras.news)) ? extras.news : [];
  const CAT_LABEL = {
    earnings_beat:'تجاوز أرباح', earnings_miss:'تقصير أرباح',
    upgrade:'رفع تصنيف', downgrade:'خفض تصنيف'
  };
  const catLines = cat.map(c =>
    `  ${CAT_LABEL[c.type]||c.type} ${c.detail||''} — قبل ${c.age_days!=null?_arNum(c.age_days):'؟'} يوم (${c.date||''})`
  ).join('\n');
  const newsLines = news.slice(0,3).map(t => `  - ${t}`).join('\n');
  const hasExtras = cat.length > 0 || news.length > 0;
  const extrasBlock = hasExtras ? `

المحفّزات الحديثة (أحداث فعلية آخر ١٤ يوم — من المحرّك، مصدرك الوحيد):
${catLines || '  لا محفّزات'}
${news.length ? `عناوين أخبار حديثة (لتفسير «لماذا» حدث المحفّز فقط — لا تكرّر أي توقّع ورد فيها):\n${newsLines}` : ''}` : '';
...
- اكتب نصاً عادياً فقط — بدون أي تنسيق ماركداون: لا عناوين (#)، لا جداول (|)، لا نجوم للتغميق (**)، لا خطوط فاصلة (---). استخدم أرقام الأجزاء (١، ٢، ٣...) وفقرات نصية بسيطة فقط.
...
الآفاق الثلاثة (٠–١٠٠، قراءات ترتيبية):
  ...
  متوسط (إعادة التسعير صعوداً): ${M.score ?? 'غير مقيَّم'}
...
${distorted?'تنبيه «درجة مدعومة بأرباح مؤقتة» (وصّل معناه بكلماتك، لا تتجاوزه): '+TERMS.flag.distortion.full:''}
...
٥) لو في تنبيه «درجة مدعومة بأرباح مؤقتة» — وصّل معناه. (إن ما فيه، تجاوز هذا الجزء.)
...
٧) السياق الشخصي كما هو أعلاه (حجم المركز، التركيز إن وُجد).${hasExtras ? `
٨) المحفّزات الحديثة: اذكر أي محفّز فعلي أعلاه بتاريخه، وإن وُجد عنوان خبر فسّر فيه «لماذا» حدث المحفّز بكلماتك — بدون تكرار أي توقّع أو رقم ورد في الخبر. إن تعارض محفّز حديث مع الدرجات، وضّح التعارض بصراحة. لا محفّز = تجاوز هذا الجزء.` : ''}
اختم بسطر تنبيه: قراءة اتجاهية بهامش خطأ واسع، ليست نصيحة. القرار قرارك.${extrasBlock}`;
}

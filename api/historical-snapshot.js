// api/historical-snapshot.js  v3
// Historical: Yahoo Finance (only source with full OHLC history)
// Current prices: FMP batch first, Yahoo fallback for misses
// Chunked fetches to stay within Vercel 10s limit
...
async function getCurrentPrices(symbols) {
  ...
  // Step 1 — FMP batch (one call, handles stocks well)
  try {
  ...
  // Step 2 — Yahoo fallback for symbols FMP missed (ETFs + small caps)
  // Fire in small chunks of 8 with 200ms gap to avoid rate limiting
  if (missing.size > 0) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Fetch Yahoo historical OHLC for ALL symbols in batches of 8
    const allOHLC = {};
    ...
    // SPY historical + current prices for ALL symbols (FMP + Yahoo fallback)
    const [spyOHLC, currentPrices] = await Promise.all([
      getYahooHistorical('SPY', date, 5),
      getCurrentPrices(symbols)   // ← replaces the old slice(0,30) + parallel Yahoo
    ]);
    ...
    if (format === 'json') {
      return res.status(200).json({
        ...
        holdings: enriched.map(h => ({
          ...
          currentPrice: currentPrices[h.sym] || null,   // ← now covers ALL symbols
          sector: h.sector
        })),
        technicals: techMap,
        currentPrices,
